            ...
            if (int.TryParse(this.txtNumPlayers.Text, out i)) {
                ...
                if (no_of_players > 2) {
                    ...
                    for (i = 0; i < no_of_players + no_of_comps; i++) {
                        switch (i) {
                            case 0:
                                frm.txtPlayer1Name.Text = new string(((player)Marshal.PtrToStructure(frm.Game.Table.players[0], typeof(player))).name);
                                continue;
                            case 1:
                                frm.txtPlayer2Name.Text = new string(((player)Marshal.PtrToStructure(frm.Game.Table.players[1], typeof(player))).name);
                                continue;
